            ...
            if(scope.question.typeId == 4 ) {
                ...
                scope.splitIntoWords = function (div) {
                    ...
                    for(var x = 0; x < elements.length ; x++){
                        if(elements[x].nodeType) {
                            var rWordBoundary = /[\s\n\t]+/; // Includes space, newline, tab
                            for (var i = 0; i < elements[x].childNodes.length; ++i) { // Iterate through all nodes
                                var node = elements[x].childNodes[i];
                                if (node.nodeType === Node.TEXT_NODE) { // The child is a text node
                                    ...
                                    for (var j = 0; j < words.length; j++) {
                                        ...
                                        if(angular.isDefined(scope.question.userAnswer)){
                                            for(var y = 0; y < scope.question.userAnswer.length; y++){
                                                if(scope.question.userAnswer[y].id == (i+words.length)*(j+1) && scope.question.userAnswer[y].value == words[j]){
                                                    addClass = true;
                                                    break;
                                                }
                                            }
                                        ...
                                        if(addClass)
                                            words[j] = "<a id='"+(i+words.length)*(j+1)+"' class='badge' ng-click='addAnswer($event)'>" + words[j] + "</a>";
                                        else
                                            words[j] = "<a id='"+(i+words.length)*(j+1)+"' ng-click='addAnswer($event)'>" + words[j] + "</a>";
                                    }
                                    if (words.length) {
                                    ...
                                    var addClass = false;
                                    if(angular.isDefined(scope.question.userAnswer)){
                                        for(var n = 0; n < scope.question.userAnswer.length; n++){
                                            if(scope.question.userAnswer[n].HTMLid == node.id &&  scope.question.userAnswer[n].value == node.innerText){
                                                addClass = true;
                                                break;
                                            }
                                        }
                                    ...
                                    if(addClass)
                                        output.push("<a  class='badge' ng-click='addAnswer($event)'>" + node.outerHTML + "</a>");
                                    else
                                        output.push("<a  ng-click='addAnswer($event)'>" + node.outerHTML + "</a>");
                                }
                            }
                            output.push("<br/>");
                        }
                    }
                    // scope.dummyContent = output.join(' ');
                    return output.join(' ');
                }
